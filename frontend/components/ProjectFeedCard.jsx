...
export default function ProjectFeedCard({ project }) {
  ...
  const shareMut = useMutation({
    mutationFn: (receiverId) => messagesAPI.send({
      receiverId,
  ...
  return (
    ...
    <div onClick={() => router.push(`/projects/${project.id}`)}
      ...
      <div className="relative h-44 bg-gray-800 overflow-hidden">
        ...
        {project.isPaid && <span className="absolute bottom-2.5 left-2.5 bg-green-900/80 text-green-400 text-[10px] px-2 py-0.5 rounded-full">💰 Paid</span>}
      </div>
