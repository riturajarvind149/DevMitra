...
function viewedKey(userId) { return `${VIEWED_KEY_PREFIX}${userId}`; }

function loadViewedMap(userId) {
...
function saveViewedMap(userId, map) {
  try {
    const obj = {};
...
function isGroupViewed(group, viewedMap) {
...
export default function StoryBar({ stories }) {
  ...
  const [shareSheetTab, setShareSheetTab] = useState("send");
  const [storySentTo, setStorySentTo]     = useState(null);
  const [shareCopied, setShareCopied]     = useState(false);

  // ── Viewed state ─────────────────────────────────────────────────────────────
  const [viewedMap, setViewedMap] = useState(new Map());
  ...
  const markViewed = useCallback((ownerId) => {
    if (!user?.id) return;
    const now = Date.now();
    setViewedMap(prev => {
  ...
  // ── Open story ────────────────────────────────────────────────────────────────
  const openStory = useCallback((group) => {
    cancelAnimationFrame(rafRef.current);
    elapsedRef.current = 0;
    setViewStory(group);
    setStoryIdx(0);
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => storiesAPI.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["activeStories"] });
