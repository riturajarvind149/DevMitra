...
function ReputationRing({ score, level, label, next }) {
...
export default function ProfilePage() {
  ...
  const earnedKeys = new Set((p?.badges ?? []).map((b) => b.badgeKey));
      ...
      <div className="hidden lg:block bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
        ...
        <div className="px-6 pb-6 relative">
          ...
          <div className="flex flex-wrap gap-3 mt-3">
            ...
            {user.linkedinUrl && <a href={user.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300"><Link2 className="h-3.5 w-3.5" />LinkedIn</a>}
            {user.availabilityHours && <span className="flex items-center gap-1 text-xs text-green-400"><Clock className="h-3.5 w-3.5" />{user.availabilityHours}h/wk available</span>}
          </div>
          {user.skills && user.skills.length > 0 && (
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start w-full min-w-0 max-w-full">
        ...
        <div className="w-full lg:flex-1 min-w-0 max-w-full space-y-4">
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-3.5 sm:p-5 w-full min-w-0 max-w-full overflow-hidden">
            ...
            {streak?.activityGrid
              ? <ActivityHeatmap grid={streak.activityGrid} />
        ...
        <div className="hidden lg:block w-72 flex-shrink-0 space-y-4 sticky top-4">
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
            ...
            <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 192, scrollbarWidth: "thin" }}>
              {allDefs.length > 0 ? allDefs.map((def) => (
